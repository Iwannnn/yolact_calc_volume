...
def evalimage(
    ...
    classes, scores, boxes, masks, img_numpy = prep_display(
    ...
    if png_save_path is None:
        ...
        cv2.imwrite(png_save_path, img_numpy)
        calc_volume(png_save_path, input_floder, name, classes, masks)
...
def evalimages(net: Yolact, input_folder: str, output_folder: str):
    ...
    for p in Path(input_folder).glob("*.png"):
        ...
        bgr_name = ".".join(name.split(".")[:-1]) + ".png"
        bgr_path = os.path.join(output_folder, bgr_name)  # 保存图片路径

        depth_name = bgr_name.split("_")[0] + "_d.pgm"
        depth_path = input_folder + "/" + depth_name

        align_bgr_img(path, depth_path)

        evalimage(
            net, path, bgr_path, input_folder, name
        )  # path : 输入图片路径 png_path : 图片输出路径。调整灰度图大小，使mask与深度图相同大小
...
def align_bgr_img(bgr_path, depth_path):
    # 读取深度图和彩色图
    depth_img = cv2.imread(depth_path, cv2.IMREAD_ANYDEPTH)
    bgr_img = cv2.imread(bgr_path)

    depth_height, depth_width = depth_img.shape[0], depth_img.shape[1]
    bgr_height, bgr_width = bgr_img.shape[0], bgr_img.shape[1]

    # 彩色图原始分辨率为1080*1920 深度图分辨率为424*512
    if depth_height == bgr_height and depth_width == bgr_width:
        return
    ...
    # ir2bgr_R = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    ...
    cv2.medianBlur(bgr_img, 5)

    cv2.imwrite(bgr_path, aligned_bgr)


def calc_volume(png_save_path, input_folder, bgr_name, classes, masks):
    if classes.size == 0:
        return
    ...
    depth_name = bgr_name.split("_")[0] + "_d.pgm"
    depth_path = input_folder + "/" + depth_name
    bgr_path = input_folder + "/" + bgr_name
    depth_save_path = png_save_path.split(".")[0] + "_depth.png"
    color_save_path = png_save_path.split(".")[0] + "_color.png"
    map_save_path = png_save_path.split(".")[0] + "_map.png"
    ply_save_path = png_save_path.split(".")[0] + "_ply.ply"
    mask_save_path = png_save_path.split(".")[0] + "_mask.png"
    masked_save_path = png_save_path.split(".")[0] + "_masked.png"
    ...
    # 将tensor.cuda 的mask转为numpy格式
    masks_np = masks[0, :, :, 0].cpu().numpy()

    save_color(depth_img, bgr_img, depth_save_path, color_save_path, map_save_path)

    # 生成点云文件
    points = generate_ply(bgr_img, depth_img)
    save_ply(points, ply_save_path)

    # 拟合边线，使用深度图构建mask
    mask = fit_edges(depth_img, masks_np)
    save_edges(mask, bgr_img, mask_save_path, masked_save_path)

    volume = calc_res(depth_img, mask)

    print("volume:", volume, "立方米")


def save_color(depth_img, bgr_img, depth_save_path, color_save_path, map_save_path):
    ...
    mapped_img = cv2.add(depth_color, bgr_img)  # 合成对齐文件

    cv2.imwrite(map_save_path, mapped_img)
    cv2.imwrite(color_save_path, depth_color)


def generate_ply(bgr_img, depth_img):
    height, width = depth_img.shape[0], depth_img.shape[1]

    camera_factor = 1000

    # 深度图像数据类型转换为uint16并将其转置，以便在后续计算中方便按行或按列遍历
    ...
    for i in range(height):
        for j in range(width):
            z = depth_img[i, j] / camera_factor  # 获取深度值
            if z > 500 and z < 4500:  # 忽略深度为0的点
                continue
            x = (i - cx) * z / fx  # 计算x坐标
            y = (j - cy) * z / fy  # 计算y坐标
            b, g, r = bgr_img[i, j]

            point = f"{x} {y} {-z} {r} {g} {b} 255\n"
            points.append(point)

    return points


def save_ply(points, ply_save_path):
    file = open(ply_save_path, "w")
    file.write(
        """ply
    format ascii 1.0
    element vertex %d
...
def fit_edges(depth_img, masks_np):
    threshold_mask = 40
    ...
    new_mask = np.zeros(depth_img.shape, np.uint8)

    # depth_img_ = depth_img.astype(np.uint8)
    depth_img = depth_img.astype(np.int16)

    # 使用条件索引找到 depth_img >= 500 and depth_img <= 4500 的元素
    filtered_depth_img = depth_img[(depth_img >= 500) & (depth_img <= 4500)]
    masked_depth_img = depth_img[masks_np != 0]

    # 找到最大值和最小值
    back_depth = np.max(filtered_depth_img)
    front_depth = np.median(masked_depth_img)

    # print("back:", back_depth)
    # print("front:", front_depth)

    new_mask = np.zeros(depth_img.shape, dtype=np.uint8)
    for i in range(new_mask.shape[0]):
        # print(depth_img[i])
        for j in range(new_mask.shape[1]):
            if (
                abs(depth_img[i, j] - front_depth) < threshold_mask
                and depth_img[i, j] > 500
                and depth_img[i, j] < 4500
                and j > 50
                and j < 472
            ):
                new_mask[i, j] = 255
    ...
    # 深度图与掩码相与，取出感兴趣的部分
    # masked_img = cv2.bitwise_and(depth_img_, depth_img_, mask=masks_np)

    # 检查yolact做的实例分割是否有空隙或者没进入的部分

    # 使用canny提取物品边线
    # new_mask = cv2.Canny(masked_img, 50, 250)  # 参数100和200分别是低阈值和高阈值

    # # 删除深度值为0的点
    # for i in range(new_mask.shape[0]):
    #     for j in range(new_mask.shape[1]):
    #         if depth_img[i, j] == 0 and new_mask[i, j] == 255:
    #             new_mask[i, j] = 0

    # # 填补左右两侧缺漏的mask
    # for i in range(new_mask.shape[0]):
    #     first_255 = -1
    #     last_255 = new_mask.shape[1]
    ...
    #     for j in range(new_mask.shape[1] - 1, -1, -1):
    #         if new_mask[i, j] == 255:
    #             last_255 = j
    #             break

    #     # 未检测到白色
    #     if first_255 == -1 and last_255 == new_mask.shape[1]:
    #         continue

    #     # 填补中间
    #     for j in range(first_255, last_255 + 1):
    #         new_mask[i, j] = 255

    #     # 填补左右
    #     for j in range(first_255 - 1, -1, -1):
    #         if abs(depth_img[i, j] - depth_img[i, j + 1]) < threshold_next:
    #             new_mask[i, j] = 255
    #         else:
    #             break
    #     for j in range(last_255 + 1, new_mask.shape[1]):
    #         if abs(depth_img[i, j - 1] - depth_img[i, j]) < threshold_next:
    #             new_mask[i, j] = 255
    #         else:
    #             break
    ...
    #     for j in range(new_mask.shape[1]):
    #         if new_mask[i, j] == 255:
    #             points = np.append(points, depth_img[i, j])

    # mid_depth = np.median(points)
    # for i in range(new_mask.shape[0]):
    #     for j in range(new_mask.shape[1]):
    #         if (
    #             new_mask[i, j] == 255
    #             and abs(depth_img[i, j] - mid_depth) > threshold_all
    #         ):
    #             new_mask[i, j] = 0

    # cv2.imshow("d_", depth_img_)
    # cv2.imshow("new_m", new_mask)
...
def save_edges(
    ...
    cv2.imwrite(mask_save_path, mask_img)  # 保存边线文件
    masked_img = cv2.cvtColor(mask_img, cv2.COLOR_GRAY2BGR)
    masked_img = cv2.add(masked_img, bgr_img)  # 合成对齐文件
    cv2.imwrite(masked_save_path, masked_img)


def calc_res(depth_img, mask):
    # mask 0为back，255为front

    back_depthes = depth_img[mask == 0]
    front_depthes = depth_img[mask == 255]

    back_depth = np.median(back_depthes)
    front_depth = np.median(front_depthes)

    target_height = back_depth - front_depth

    pixel_size, pixel_num = calc_pixel_size(depth_img, mask)

    volume = pixel_num * pixel_size * target_height

    print("h:", target_height)
    print("s:", pixel_size)
    print("n:", pixel_num)
...
def calc_pixel_size(depth_img, mask):
    fx, fy, cx, cy = 365.4947, 365.1759, 256.7344, 202.3221
    ...
    for i in range(mask.shape[0]):
        white_pixel = cv2.countNonZero(mask[i])
        white_pixel_num += white_pixel
        yy1, yy2 = 0, 0
        flag = 0  # flag为0则当前计算的白色像素不连续，为1则连续
    ...
    # 获取两点深度值
    z1 = int(depth_img[x1, y1])
    z2 = int(depth_img[x2, y2])
    ...
    pixel_length = dist / pixel_num
    pixel_size = pixel_length * pixel_length

    return pixel_size, white_pixel_num
